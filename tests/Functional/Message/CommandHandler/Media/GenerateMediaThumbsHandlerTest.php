...
class GenerateMediaThumbsHandlerTest extends TestCase
{
    ...
    public function it_generates_all_thumbnails()
    {
        ...
        $this->generateMediaThumbsHandler->__invoke($command);
        $media = $this->mediaManager->reload($media);
        self::assertCount(3, $media->getThumbsInfo());

        $provider = $this->pool->getProvider($media->getProviderName());
        foreach ($provider->getFormats() as $format) {
            $this->assertTrue($this->thumbnail->isThumbExists($provider, $media, $format));
        }
    }
